...
from .publishing import FilePublisher, WeChatPublisher, XhsPublisher, render_package
...
def main() -> None:
    ...
    publish.add_argument("--platform", required=True, choices=["xhs", "wechat", "zhihu_answer", "x_thread", "jike"]); publish.add_argument("--staged-id", required=True)
    ...
    if args.command == "source-jsonl":
        ...
        package = render_package(_json(args.paper), args.platform, args.canonical_url)
        if args.platform == "wechat":
            publisher = WeChatPublisher(args.data_root)
        elif args.platform == "xhs":
            publisher = XhsPublisher(args.data_root)
        else:
            publisher = FilePublisher(args.platform, args.data_root)
        print(json.dumps(publisher.stage(package), ensure_ascii=False, indent=2))
        ...
        if args.platform == "wechat":
            ...
            publisher = FilePublisher(args.platform, args.data_root)
        print(json.dumps(publisher.publish(args.staged_id), ensure_ascii=False, indent=2))
