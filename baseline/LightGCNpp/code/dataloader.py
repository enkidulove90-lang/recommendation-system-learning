...
class Loader(BasicDataset):
    ...
    def __init__(self,config = world.config,path="../data/gowalla"):
        ...
        # ---- idea2: 加载多模态视觉特征(行序 = item id, 与 image_feat.npy 约定一致) ----
        self.mm_feats = None
        if world.config.get('use_mm', 0):
            self._load_mm_feats(path)

    def _load_mm_feats(self, path):
        """加载 {image_feat.npy, image_feat_clip.npy, ...} 到 GPU/CPU.
        每个 .npy 必须是 [n_items, feat_dim] 且第 i 行 = item i.
        文件名去掉 .npy 后缀即为类型名(如 image_feat → 'image_feat')."""
        import glob as _glob
        feats = {}
        cands = _glob.glob(os.path.join(path, '*.npy'))
        # 排除非特征文件(如邻接缓存 s_pre_adj_mat*.npz 不在此目录, 但保险起见过滤)
        for f in cands:
            name = os.path.splitext(os.path.basename(f))[0]
            if name.startswith('s_pre_adj'):
                continue
            arr = np.load(f)
            arr = torch.tensor(arr, dtype=torch.float32, device=world.device)
            feats[name] = arr
        if not feats:
            raise FileNotFoundError(f"use_mm=1 but no .npy feature found in {path}")
        self.mm_feats = feats
        print(f"[idea2] loaded mm_feats from {path}: "
              + ", ".join(f"{k}{tuple(v.shape)}" for k, v in feats.items()))
