...
- **数据与任务设置**：数据集：Amazon Reviews数据集：Sports & Outdoors、Amazon Reviews数据集：Toys & Games、Amazon Reviews数据集：Beauty & Personal Care、Amazon Reviews数据集：Arts, Crafts & Sewing；任务：Experiment In this section, we analyze the efectiveness of ItemRAG in the LLMbased recommendation tasks.；划分：leave-one-out
- **基线与对照**：We further conduct a Wilcoxon signed-rank test between ItemRAG and each baseline to assess statistical significance.；Baseline methods and ItemRAG.；For comparison, we use 9 baseline methods: two graph-based models (LightGCN [2] and Light-GCN++ [15]), two sequential models (SASRec [5] and BERT4Rec [20]), one naive zero-shot LLM-based recommender, and four user-based RAG methods (ICL [21], AdaptRec [25], ReACT [4], and CoRAL [23]).
- **评价指标**：原文未披露
- **实现环境与关键参数**：原文未披露
