...
- **数据与任务设置**：数据集：未明确指定单一基准数据集，但提及了工业级平台如YouTube、Amazon、Spotify、TikTok、Alibaba、Meta等的实践案例；任务：4.3 Evaluation and Experimentation Challenges Evaluation in recommender systems is inherently constrained by the ofline-online discrepancy, label sparsity, and the delayed nature of user feedback.
- **基线与对照**：LLMs also enable <sub>pairwise</sub> <sub>comparison</sub> of ranking slates.；What do you do next?</td></tr><tr><td>Pairwise Slate Comparison</td><td>Which list is better for the user based on relevance and novelty?；<sub>4.3.2</sub> <sub>Sparse</sub> <sub>Conversion</sub> <sub>Labels.</sub> Many downstream objectives—such as purchases, subscriptions, or upgrades—occur with extremely low frequency compared to higher-volume user actions like impressions or clicks.
- **评价指标**：NDCG、RECALL、MAP
- **实现环境与关键参数**：原文未披露
